...
export class TokenService {
  ...
  public async generateAuthToken(payload: JwtPayload): Promise<TokenDto> {
    const expiresIn = this.configService.get(EnvVariables.jwt.accessExpiresIn);
    ...
    const accessToken = this.generateToken(payload, `${expiresIn}s`);
    const refreshToken = this.generateToken(payload, `${refreshTokenExpires}d`);
